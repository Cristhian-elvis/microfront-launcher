...
import {
  ...
  saveProject, hideProject, getVersions, getTags, refreshTags, versionPaths, safeTagName,
  assertInside, readJson, needsInitialSetup, completeInitialSetup, invalidateProjectScan, getIndexedProject
} from './lib/core.js';
import {
  childProcesses, eventClients, logs, emit, addLog,
  processStatus, spawnManaged, stopProcess, waitForUrl,
  inspectHttp, startStaticServer, stopStaticServer, stopAllStaticServers,
  serverStatus
} from './lib/runtime.js';
...
const state = {
  session: { status: 'idle', stage: 'idle', projectId: null, projectName: null, message: 'Todo detenido', startedAt: null },
  components: { status: 'stopped', url: null, version: null, tag: null, external: false },
  shell: { status: 'stopped', url: null, projectId: null, name: null, appName: null, external: false },
  microfrontend: { status: 'stopped', projectId: null, id: null, name: null, version: null },
  microfrontendBranch: { status: 'idle', projectId: null, microfrontendId: null, phase: null, message: null, error: null, startedAt: null },
  microfrontendBuild: { status: 'idle', projectId: null, microfrontendId: null, phase: null, message: null, error: null, startedAt: null },
  microfrontendOperations: {},
  build: { status: 'idle', tag: null, action: null, message: 'Sin compilaciones en curso', startedAt: null },
  execution: { status: 'idle', projectId: null, projectName: null, steps: [], error: null, startedAt: null, endedAt: null },
  browserPrompt: null
};
...
function updateMicrofrontendOperation(kind, projectId, microfrontendId, patch) {
  ...
  state.microfrontendOperations = {
    ...state.microfrontendOperations,
    [key]: { ...(state.microfrontendOperations[key] || {}), kind, projectId, microfrontendId, ...patch }
  };
  emitState();
}
...
function beginExecution(project, steps, kind = 'start') {
  state.execution = {
    status: 'running', kind, projectId: project.id, projectName: project.name,
    steps: steps.map((step) => ({ ...step, status: 'pending', message: null, startedAt: null, endedAt: null })),
...
function updateExecutionStep(id, patch) {
  state.execution = {
    ...state.execution,
    steps: state.execution.steps.map((step) => step.id === id ? { ...step, ...patch } : step)
  };
  emitState();
}
...
function recordStartValidationError(project, error) {
  ...
  state.execution = {
    status: 'error', kind: 'start', projectId: project.id, projectName: project.name,
    steps: [{
      id: 'validation', label: 'Validar inicio', command: 'Validar shell y MOVA Components',
      status: 'error', message: error.message, startedAt: now, endedAt: now
    }],
    error: error.message, startedAt: now, endedAt: now
  };
  emitState();
}
...
async function runTrackedStage({ id, message, task }) {
  ...
  try {
    ...
    updateExecutionStep(id, { status: 'error', message: error.message, endedAt: new Date().toISOString() });
...
function selectedVersion() {
  ...
  return versions.find((item) => item.tag === preferences.preferredTag)
...
function beginOperation(type, details = {}) {
  ...
  if (key === 'buildOperation' ? buildOperation : environmentOperation) throw new Error(type === 'build' ? 'Ya hay una compilación en curso.' : 'Ya hay una operación de entorno en curso.');
  const controller = new AbortController();
  const next = { id: `${Date.now()}-${Math.random()}`, type, controller, ...details };
...
async function runProcessStep({ key, label, file, args, cwd, signal }) {
  ...
  assertNotCancelled(signal);
  if (result.code !== 0) throw new Error(`${label} terminó con código ${result.code}.`);
}

async function buildComponents(version, signal, { force = false } = {}) {
  const tag = version.tag;
  const available = getTags();
  if (!available.some((item) => item.tag === tag)) throw new Error(`El tag no existe localmente: ${tag}`);
  ...
  for (const directory of storageDirectories) {
    ...
    addLog('Versiones', 'stage', `Creando ${directory.label}: ${directory.path}`);
  ...
  try {
    await runProcessStep({ key: 'build:clone', label: 'Versiones', file: 'git', args: ['clone', '--local', '--no-checkout', config.mova.sourcePath, working], cwd: workRoot, signal });
    await runProcessStep({ key: 'build:checkout', label: 'Versiones', file: 'git', args: ['checkout', '--detach', tag], cwd: working, signal });
    await runProcessStep({ key: 'build:install', label: 'Build MOVA', file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['ci'], cwd: working, signal });
    await runProcessStep({ key: 'build:compile', label: 'Build MOVA', file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'build'], cwd: working, signal });
    assertNotCancelled(signal);

    const sourceDist = path.join(working, 'dist');
    if (!fs.existsSync(sourceDist)) throw new Error('El build no generó la carpeta dist.');
    ...
    addLog('Build MOVA', 'success', `${tag} compilado y almacenado para reutilizarse.`);
    return destination;
  } catch (error) {
    throw error;
  } finally {
    if (!fs.existsSync(destination.manifest) && fs.existsSync(destination.root)) {
      assertInside(versionsRoot, destination.root);
      fs.rmSync(destination.root, { recursive: true, force: true });
    }
...
async function compileVersion(tag) {
  ...
  updateBuild({ status: 'building', tag, action: 'compile', message: `Compilando ${tag}`, startedAt: new Date().toISOString() });
  try {
    await buildComponents(version, current.controller.signal, {
      force: version.cached,
    });
    updateBuild({ status: 'success', message: `${tag} compilado correctamente`, startedAt: null });
  } catch (error) {
    updateBuild({ status: current.controller.signal.aborted ? 'idle' : 'error', message: current.controller.signal.aborted ? 'Compilación cancelada' : error.message, startedAt: null });
    throw error;
  } finally {
...
function cancelBuild() {
  if (!buildOperation) throw new Error('No hay una compilación de MOVA en curso.');
  addLog('Build MOVA', 'system', `Cancelando compilación de ${buildOperation.tag}.`);
...
async function ensureComponents(project, version, signal) {
  ...
  const link = path.join(project.serverPath, 'cudc-lib-componentes-stencil-VAL');
  ...
  if (fs.existsSync(link)) {
    ...
    if (info.isSymbolicLink() || info.isDirectory()) fs.rmSync(link, { recursive: true, force: true });
  }
  assertNotCancelled(signal);
  ...
  state.components = { status: 'running', url: project.url, version: version.version, tag: version.tag, external: false };
...
async function startComponentsStandalone() {
  ...
  if (!version) throw new Error('Selecciona una versión de MOVA Components antes de iniciarlo.');
  const paths = await buildComponents(version, new AbortController().signal);
  const config = readConfig();
  const server = await startStaticServer({ key: 'components', label: 'MOVA Components', root: paths.dist, port: Number(config.shellDefaults.serverPort), aliases: ['/cudc-lib-componentes-stencil-VAL'], fallbackIndex: false });
  state.components = { status: 'running', url: server.url, version: version.version, tag: version.tag, external: false };
  emitState();
...
function startMicrofrontend(project, microfrontend, signal) {
  assertNotCancelled(signal);
  if (!microfrontend?.watchAvailable) throw new Error(`El microfrontend ${microfrontend?.name || ''} no define npm run watch.`);
  const key = `microfrontend:${project.id}:${microfrontend.id}`;
  state.microfrontend = { status: 'starting', projectId: project.id, id: microfrontend.id, name: microfrontend.name, version: microfrontend.version };
  emitState();
  const record = spawnManaged({ key, label: microfrontend.name, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'watch'], cwd: microfrontend.path });
  record.done.then(() => {
...
function buildMicrofrontend(project, microfrontend) {
  ...
  if (!microfrontend?.buildAvailable) throw new Error(`El microfrontend ${microfrontend?.name || ''} no define npm run build.`);
  const key = `microfrontend-build:${project.id}:${microfrontend.id}`;
  updateMicrofrontendBuild({ status: 'running', projectId: project.id, microfrontendId: microfrontend.id, phase: 'build', message: 'Compilando.', error: null, startedAt: new Date().toISOString() });
  ...
  record.done.then(({ code }) => {
    ...
    if (code === 0) {
      updateMicrofrontendBuild({ status: 'success', message: `${microfrontend.name} compilado correctamente.`, error: null });
    } else {
      updateMicrofrontendBuild({ status: 'error', message: 'La compilación no se completó.', error: `${microfrontend.name} terminó con código ${code ?? '-'}.` });
    }
  });
  return record;
}
...
function startMicrofrontendBuildBatch(projectId, microfrontendIds) {
  ...
  const run = async () => {
    const results = await Promise.all(microfrontends.map(async (microfrontend) => {
      updateMicrofrontendOperation('build', projectId, microfrontend.id, { status: 'running', phase: 'build', message: 'Compilando', error: null, startedAt });
      const record = spawnManaged({ key: `microfrontend-build:${project.id}:${microfrontend.id}`, label: `Build · ${microfrontend.name}`, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'build'], cwd: microfrontend.path, longRunning: false });
      const { code } = await record.done;
      if (code === 0) { updateMicrofrontendOperation('build', projectId, microfrontend.id, { status: 'success', phase: 'done', message: 'Completado', error: null, startedAt }); return true; }
      const error = `Terminó con código ${code ?? '-'}.`;
      updateMicrofrontendOperation('build', projectId, microfrontend.id, { status: 'error', phase: 'error', message: 'No se completó la compilación', error, startedAt });
      return false;
    }));
    const completedIds = microfrontends.filter((_, index) => results[index]).map((item) => item.id);
    updateMicrofrontendBuild({ status: results.every(Boolean) ? 'success' : 'error', projectId, microfrontendId: microfrontends.at(-1).id, phase: 'done', message: `${completedIds.length}/${microfrontends.length} microfronts compilados.`, error: null, startedAt, batchIds: microfrontendIds, completedIds });
  };
  microfrontendBatchOperation = run().catch((error) => {
    updateMicrofrontendBuild({ status: 'error', error: error.message, message: 'La compilación por lote no se completó.', startedAt, batchIds: microfrontendIds });
...
function startMicrofrontendBranchBatch(projectId, microfrontendIds, branch) {
  ...
  const run = async () => {
    const results = await Promise.all(microfrontends.map(async (microfrontend) => {
      const report = (patch) => updateMicrofrontendOperation('branch', projectId, microfrontend.id, { status: 'running', branch, startedAt, ...patch });
      report({ phase: 'validating', message: 'Preparando cambio', error: null });
      try { await switchMicrofrontendBranch(projectId, microfrontend.id, branch, null, report); updateMicrofrontendOperation('branch', projectId, microfrontend.id, { status: 'success', branch, phase: 'done', message: 'Completado', error: null, startedAt }); return true; }
      catch (error) { updateMicrofrontendOperation('branch', projectId, microfrontend.id, { status: 'error', branch, phase: 'error', message: 'No se pudo cambiar la rama', error: error.message, startedAt }); return false; }
    }));
    const completedIds = microfrontends.filter((_, index) => results[index]).map((item) => item.id);
    updateMicrofrontendBranch({ status: results.every(Boolean) ? 'success' : 'error', projectId, microfrontendId: microfrontends.at(-1).id, branch, phase: 'done', message: `Rama ${branch} aplicada a ${completedIds.length}/${microfrontends.length} microfronts.`, error: null, startedAt, batchIds: microfrontendIds, completedIds });
  };
  branchOperation = run().catch((error) => {
    updateMicrofrontendBranch({ status: 'error', branch, error: error.message, message: 'El cambio de rama por lote no se completó.', startedAt, batchIds: microfrontendIds });
    addLog('Microfronts', 'error', error.message);
  }).finally(() => { branchOperation = null; });
}
...
async function startShell(project, signal) {
  ...
  if (!project.configured || !project.appName) throw new Error('La shell no tiene una aplicación enlazada. Ejecuta primero scaffolding.');
  ...
  if (occupied.reachable) throw new Error(`El puerto ${configuredPort} ya está ocupado por otra shell o servicio.`);
  ...
  state.shell = { status: 'starting', url: shellUrl.toString(), projectId: project.id, name: project.name, appName: project.appName, external: false };
  emitState();
  ...
  state.shell = { status: 'running', url: shellUrl.toString(), projectId: project.id, name: project.name, appName: project.appName, external: false };
  emitState();
}
...
function browserSettings(config = readConfig()) {
  ...
  return { name: isEdge ? 'Edge' : 'Chrome', path: isEdge ? config.chrome.edgePath : config.chrome.path, userDataDir: isEdge ? config.chrome.edgeUserDataDir : config.chrome.userDataDir, insecure };
}

function hasBrowserProfileOpen(browser) {
  if (process.platform !== 'win32' || !browser.insecure) return Promise.resolve(false);
  ...
  return new Promise((resolve) => execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { windowsHide: true }, (error, stdout) => resolve(!error && /^true\s*$/i.test(stdout))));
}

async function openChrome(project, { newWindow = true } = {}) {
  const config = readConfig();
  const browser = browserSettings(config);
  if (!fs.existsSync(browser.path)) throw new Error(`No se encontró ${browser.name}: ${browser.path}`);
  const chromeUrl = project ? new URL(project.url) : new URL('chrome://newtab/');
  if (project) { chromeUrl.hostname = config.chrome.openHost; chromeUrl.port = String(config.shellDefaults.serverPort || 8080); }
  const args = [
    ...(browser.insecure ? [`--user-data-dir=${browser.userDataDir}`, '--disable-web-security'] : []),
    ...(newWindow ? ['--new-window'] : []), chromeUrl.toString()
  ];
  await new Promise((resolve, reject) => {
    const chrome = spawn(browser.path, args, { detached: true, stdio: 'ignore', windowsHide: false });
...
async function openOrRequestBrowser(project) {
  ...
  if (config.chrome.openMode) return openChrome(project, { newWindow: config.chrome.openMode === 'window' });
  state.browserPrompt = { projectId: project.id, browser: browser.name, insecure: browser.insecure };
  addLog(browser.name, 'system', 'Ya hay una ventana con esta configuración. Esperando la elección de apertura.');
  emitState();
}
...
async function runEnvironment(projectId, options = {}) {
  ...
  updateExecutionStep('validation', { status: 'running', startedAt: validationStartedAt });
  ...
  try {
    ...
    if (includeComponents && !version) throw new Error('No hay versiones de MOVA UI Components disponibles.');
    microfrontend = options.microfrontendId ? project.microfrontends?.find((item) => item.id === options.microfrontendId) : null;
    ...
    updateExecutionStep('validation', { status: 'success', endedAt: new Date().toISOString() });
  } catch (error) {
    recordStartValidationError(project, error);
    throw error;
  }
  if (state.components.status === 'running') {
    addLog('MOVA Components', 'stage', 'Deteniendo Components independiente para iniciar la shell en el mismo puerto.');
    await stopComponents();
    addLog('MOVA Components', 'success', 'Components independiente detenido.');
  }
  const current = beginOperation('environment', { projectId, microfrontendId: microfrontend?.id || null });
  const signal = current.controller.signal;
  const plan = [...(includeComponents ? ['components'] : []), ...(automaticBuild ? ['prepare', 'buildShell'] : []), ...(microfrontend ? ['microfrontend'] : []), 'shell'];
  const steps = [
    { id: 'validation', label: 'Validar inicio', command: 'Validar shell y MOVA Components', detail: 'Comprobando el build local y la configuración requerida.' },
    ...(includeComponents ? [{ id: 'components', label: 'MOVA Components', command: 'Compilar Components en la shell', detail: 'Instalando la librería dentro de server/cudc-lib-componentes-stencil-VAL' }] : []),
    ...(automaticBuild ? [
      { id: 'prepare', label: 'Preparar servidor', command: 'Preparar servidor de la shell', detail: 'Ejecutando: npm run prepare-server' },
      { id: 'buildShell', label: 'Compilar shell', command: 'Construir shell local', detail: 'Ejecutando: npm run build:local' }
    ] : []),
    ...(microfrontend ? [{ id: 'microfrontend', label: microfrontend.name, command: 'Iniciar watch del microfrontend', detail: 'Ejecutando: npm run watch' }] : []),
    { id: 'shell', label: project.appName || project.name, command: 'Iniciar shell', detail: 'Servidor HTTP interno del launcher (Node.js)' },
    { id: 'chrome', label: 'Chrome autorizado', command: 'Abrir navegador autorizado', detail: 'Ejecutando navegador autorizado' }
  ];
  beginExecution(project, steps);
  updateExecutionStep('validation', { status: 'success', startedAt: validationStartedAt, endedAt: new Date().toISOString() });
  updateSession({ status: 'starting', stage: includeComponents ? 'components' : 'shell', plan, message: includeComponents ? `Iniciando MOVA Components ${version.version}` : `Iniciando ${project.name} sin MOVA Components`, projectId, projectName: project.name, startedAt: new Date().toISOString() });

  try {
    if (includeComponents) await runTrackedStage({ id: 'components', message: `Compilando MOVA Components ${version.version}`, task: async () => { assertNotCancelled(signal); await ensureComponents(project, version, signal); assertNotCancelled(signal); } });
    if (automaticBuild) {
      await runTrackedStage({ id: 'prepare', message: `Preparando enlaces de ${project.appName || project.name}`, task: () => runProcessStep({ key: `prepare:${project.id}`, label: `${project.name} · prepare-server`, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'prepare-server'], cwd: project.path, signal }) });
      await runTrackedStage({ id: 'buildShell', message: `Compilando shell local ${project.appName || ''}`, task: () => runProcessStep({ key: `build-shell:${project.id}`, label: `${project.name} · build:local`, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'build:local'], cwd: project.path, signal }) });
    }
    if (microfrontend) {
      await runTrackedStage({ id: 'microfrontend', message: `Iniciando watch de ${microfrontend.name}`, task: async () => { assertNotCancelled(signal); startMicrofrontend(project, microfrontend, signal); } });
    }
    await runTrackedStage({ id: 'shell', message: `Iniciando servidor HTTP de ${project.name}`, task: async () => { assertNotCancelled(signal); await startShell(project, signal); assertNotCancelled(signal); } });
    await runTrackedStage({ id: 'chrome', message: 'Abriendo navegador autorizado.', task: async () => { assertNotCancelled(signal); await openOrRequestBrowser(project); assertNotCancelled(signal); } });
    updateExecution({ status: 'success', error: null, endedAt: new Date().toISOString() });
    ...
    updateExecution({
      status: finalStatus, error: cancelled ? 'Inicio cancelado' : error.message, endedAt: new Date().toISOString(),
      steps: state.execution.steps.map((step) => ({
        ...step,
        status: step.status === 'pending' ? 'skipped' : cancelled && ['running', 'error'].includes(step.status) ? 'cancelled' : step.status,
        endedAt: ['running', 'pending'].includes(step.status) ? new Date().toISOString() : step.endedAt
      }))
    });
    await cleanupStartedResources();
    updateSession({ status: 'idle', stage: 'idle', plan: null, message: cancelled ? 'Inicio cancelado' : `Error: ${error.message}`, projectId: null, projectName: null, startedAt: null });
  } finally {
    endOperation(current.id);
  }
}
...
async function rebuildShellServer(projectId) {
  ...
  const current = beginOperation('environment', { projectId, mode: 'rebuild-server' });
  const signal = current.controller.signal;
  const steps = [
    { id: 'prepare', label: 'Preparar servidor', command: 'Preparar servidor de la shell', detail: 'Ejecutando: npm run prepare-server' },
    { id: 'buildShell', label: 'Compilar shell', command: 'Construir servidor local', detail: 'Ejecutando: npm run build:local' }
  ];
  ...
  try {
    await runTrackedStage({ id: 'prepare', message: 'Preparar servidor de la shell', task: () => runProcessStep({ key: `prepare:${project.id}`, label: `${project.name} · prepare-server`, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'prepare-server'], cwd: project.path, signal }) });
    await runTrackedStage({ id: 'buildShell', message: 'Construir servidor local', task: () => runProcessStep({ key: `build-shell:${project.id}`, label: `${project.name} · build:local`, file: process.platform === 'win32' ? 'npm.cmd' : 'npm', args: ['run', 'build:local'], cwd: project.path, signal }) });
    updateExecution({ status: 'success', error: null, endedAt: new Date().toISOString() });
    ...
    const cancelled = signal.aborted;
    updateExecution({ status: cancelled ? 'cancelled' : 'error', error: cancelled ? 'Reconstrucción cancelada' : error.message, endedAt: new Date().toISOString() });
...
async function cancelAndStopAll(reason = 'Entorno detenido') {
  ...
  const stopSteps = [
    ...(state.shell.status !== 'stopped' ? [{ id: 'stopShell', label: projectName, command: 'Detener shell', detail: 'Cerrar servidor HTTP interno' }] : [])
  ];
  state.execution = { status: 'running', kind: 'stop', projectId: state.shell.projectId, projectName, steps: stopSteps.map((step) => ({ ...step, status: 'pending', startedAt: null, endedAt: null })), error: null, startedAt: new Date().toISOString(), endedAt: null };
  ...
  try {
    if (state.shell.status !== 'stopped') await stopStep('stopShell', projectName, 'Deteniendo servidor HTTP de la shell.', async () => {
...
function openScaffolding(projectId) {
  ...
  const child = process.platform === 'win32'
    ? spawn('cmd.exe', ['/k', 'npm run scaffolding'], { cwd: project.path, detached: true, stdio: 'ignore', windowsHide: false })
    : spawn('npm', ['run', 'scaffolding'], { cwd: project.path, detached: true, stdio: 'ignore' });
  child.unref();
  addLog(project.name, 'system', 'Configuración inicial abierta en una terminal independiente.');
}
...
async function openMicrofrontendFolder(projectId, microfrontendId) {
  ...
  addLog('Microfronts', 'system', `Carpeta abierta para ${microfrontend.name}.`);
}
...
async function switchMicrofrontendBranch(projectId, microfrontendId, branch, progress = null, report = null) {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._/-]*$/.test(branchName)) throw new Error('La rama indicada no es válida.');
  const progressMessage = (message, step) => {
    const batch = progress?.total > 1 ? ` · microfront ${progress.position}/${progress.total}` : '';
    return `${message} (paso ${step}/4${batch})`;
  };
  const updatePhase = (phase, message) => { updateMicrofrontendBranch({ phase, message }); report?.({ phase, message }); };
  ...
  if (changes) throw new Error(`${microfrontend.name} tiene cambios locales. Confírmalos o guárdalos antes de cambiar de rama.`);
  addLog('Microfronts', 'stage', `Actualizando referencias de ${microfrontend.name}.`);
  updatePhase('fetch', progressMessage('Actualizando referencias remotas', 2));
  await runGit(['fetch', '--all', '--prune'], microfrontend.path);
  const localBranches = await runGit(['branch', '--format=%(refname:short)'], microfrontend.path);
  const hasLocalBranch = localBranches.split(/\r?\n/).includes(branchName);
  const remoteBranches = await runGit(['branch', '--remotes', '--format=%(refname:short)'], microfrontend.path);
  const hasRemoteBranch = remoteBranches.split(/\r?\n/).includes(`origin/${branchName}`);
  if (!hasLocalBranch && !hasRemoteBranch) throw new Error(`La rama ${branch} no existe en el repositorio de ${microfrontend.name}.`);
  updatePhase('switch', progressMessage(`Cambiando a ${branchName}`, 3));
  await runGit(hasLocalBranch ? ['switch', branchName] : ['switch', '--track', `origin/${branchName}`], microfrontend.path);
  if (hasRemoteBranch) {
    updatePhase('pull', progressMessage(`Sincronizando ${branchName}`, 4));
    await runGit(['pull', '--ff-only', 'origin', branchName], microfrontend.path);
  }
  invalidateProjectScan();
  addLog('Microfronts', 'success', `${microfrontend.name} sincronizado en la rama ${branch}.`);
}
...
function startMicrofrontendBranchSwitch(projectId, microfrontendId, branch) {
  ...
  branchOperation = { id };
  updateMicrofrontendBranch({ status: 'running', projectId, microfrontendId, branch, phase: 'validating', message: 'Preparando cambio de rama.', error: null, startedAt: new Date().toISOString() });
  switchMicrofrontendBranch(projectId, microfrontendId, branch, { position: 1, total: 1 })
    .then(() => updateMicrofrontendBranch({ status: 'success', branch, phase: 'done', message: `Rama ${branch} aplicada correctamente.`, error: null }))
    .catch((error) => {
      addLog('Microfronts', 'error', error.message);
      updateMicrofrontendBranch({ status: 'error', phase: 'error', message: 'No se pudo cambiar la rama.', error: error.message });
    })
    .finally(() => { if (branchOperation?.id === id) branchOperation = null; emitState(); });
}

function selectLocalDirectory(description) {
  if (process.platform !== 'win32') throw new Error('El selector de carpetas solo está disponible en Windows.');
  const script = [
  ...
  return new Promise((resolve, reject) => execFile('powershell.exe', ['-NoProfile', '-STA', '-Command', script], { windowsHide: false }, (error, stdout) => {
...
const handleMicrofrontendRequest = createMicrofrontendHandler({
  ...
  stopWatch: (projectId, microfrontendId) => stopProcess(`microfrontend:${projectId}:${microfrontendId}`),
  readBody, json,
});
...
const handleBrowserRequest = createBrowserHandler({
  state, getProjects, openOrRequestBrowser, reopenChrome, openEmptyBrowser,
  readConfig, writeJson, configPath, emitState, readBody, json,
});
const handleStateRequest = createStateHandler({
  getState, readConfig, needsInitialSetup, getProjects, logs, eventClients, json,
});
const handleMovaRequest = createMovaHandler({
  getVersions, getTags, refreshTags, writePreferences, emitState, addLog, compileVersion, cancelBuild, readBody, json,
});
const handleComponentsRequest = createComponentsHandler({
  state, startComponents: startComponentsStandalone, stopComponents, stopEnvironment: cancelAndStopAll, json,
});
const handleEnvironmentRequest = createEnvironmentHandler({
  runEnvironment, rebuildShellServer, cancelAndStopAll, addLog, readBody, json,
});
const routeApi = createApiRouter([
  handleStateRequest, handleProjectsRequest, handleMovaRequest, handleComponentsRequest,
  handleBrowserRequest, handleMicrofrontendRequest, handleEnvironmentRequest,
]);
...
function serveInterface(response, pathname) {
  ...
  if (relation.startsWith('..') || path.isAbsolute(relation)) return json(response, 403, { error: 'Acceso denegado' });
  if (!fs.existsSync(filePath) || fs.statSync(filePath).isDirectory()) filePath = path.join(distRoot, 'index.html');
  if (!fs.existsSync(filePath)) return json(response, 503, { error: 'Ejecuta npm run build.' });
...
server.on('error', (error) => {
  console.error(error.code === 'EADDRINUSE' ? `El puerto ${port} ya está en uso.` : error);
  process.exit(1);
});
...
if (process.argv.includes('--check')) {
  ...
  server.listen(port, host, () => {
    ...
    if (process.env.NO_OPEN !== '1') {
      const browser = spawn('cmd.exe', ['/c', 'start', '', `http://${host}:${port}`], { detached: true, stdio: 'ignore', windowsHide: true });
      browser.unref();
    }
  });
  process.on('SIGINT', async () => { await cancelAndStopAll('Launcher cerrado'); server.close(() => process.exit(0)); });
  process.on('SIGTERM', async () => { await cancelAndStopAll('Launcher cerrado'); server.close(() => process.exit(0)); });
}
